async function runEvaluation() {
  ...
  // Get selected models
  const modelBoxes = document.querySelectorAll('.checkbox-group input[type=checkbox]');
  const models = [];
  const metrics = [];

  document.querySelectorAll('.card:nth-child(2) input:checked').forEach(cb => models.push(cb.value));
  document.querySelectorAll('.card:nth-child(3) input:checked').forEach(cb => metrics.push(cb.value));
...
function renderResults(results) {
  ...
  if (!results || results.length === 0) {
    container.innerHTML = '<p style="color:#aaa;text-align:center;">No results returned.</p>';
    return;
  }
